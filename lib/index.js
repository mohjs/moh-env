...
const loadAllConfigs = (folder, encoding) => {
  ...
  configFiles.map(fileName => fileName.split('.')[0]).map((envName, index) => {
...
const generateValue = (itemName, key, vals) => {
  ...
  const existDefaultValue =
    defaultConfig[itemName][key][2] ||
  ...
  if (requiredType && existDefaultValue) {
    const confValue =
      (exist(P) ? process.env[defaultConfig[itemName][key][1]] : null) ||
...
const init = (optionPath = null, optionEncode = null) => {
  ...
  if (isInitialized) return finalConfig
  ...
  if (fs.existsSync(configFolder)) {
    ...
    if (!finalConfig) {
      warn(
        `>>> No configs found for [${
          process.env.NODE_ENV
        }], please check the configs rule.`
      )
    } else {
      isInitialized = true
    ...
    warn(
      `>>> No folder [${configFolder}] found in the path. Failed to load env configs...`
    )
  }
}
