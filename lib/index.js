...
const generateConfigsAndDesc = (defaultConfig, envConfig) => {
  Object.keys(defaultConfig).map(itemName => {
    Object.keys(defaultConfig[itemName]).reduce((config, key) => {
      ...
      if (envConfig[itemName]) {
        if (defaultValue) {
          config[itemName][key] = process.env[defaultConfig[itemName][key][1]] || envConfig[itemName][key] || defaultValue
        } else {
          const keyValue = process.env[defaultConfig[itemName][key][1]] || envConfig[itemName][key]
          if (keyValue) {
        ...
        if (defaultValue) {
          config[itemName][key] = process.env[defaultConfig[itemName][key][1]] || defaultValue
        } else {
          warn(`>>> Env config ${defaultConfig[itemName][key][1] ? defaultConfig[itemName][key][1] : `[${itemName}][${key}]`} has no value.`)
        }
      }
